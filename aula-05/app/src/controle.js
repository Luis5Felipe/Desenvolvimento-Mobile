...
const Controle = function () {
  const jogar = (resposta) => {
    document.getElementById("tela1").value =
      `${Dados.programa}->${Dados.pergunta}:${resposta}`;

    if (resposta == Dados.gabarito[Dados.pergunta - 1]) {
      document.getElementById("tela1").style.border = "green 3px solid";

      if (Dados.tentativa == 1) Dados.pontos += 3;
      else if (Dados.tentativa == 2) Dados.pontos += 2;
      else if (Dados.tentativa == 3) Dados.pontos++;
